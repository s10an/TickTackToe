...
export class matchStatistics{
    ...
    public static updateResult = (completeGame : Array<board>, playerStarting : iPlayer):void =>{
        let finalBoard = completeGame[completeGame.length-1];
        let gamestring : string = playerStarting.playerName + " marker: " + playerStarting.playerMarker + " | ";
        finalBoard.moveSequence.forEach( m => {
            gamestring += m.moveIndex.toString() + ",";
        });
        
        gamestring += " | " + finalBoard.GameStatus.toString()
        matchStatistics.stats.moves.push(gamestring);
